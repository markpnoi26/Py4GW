...
def _IsReadyToCast(slot, 
        ...
        current_energy = GetEnergyValues_fn(GLOBAL_CACHE.Player.GetAgentID()) * GLOBAL_CACHE.Agent.GetMaxEnergy(GLOBAL_CACHE.Player.GetAgentID())
...
class CombatClass:
    ...
    def AreCastConditionsMet(self, slot, vTarget):
        return _AreCastConditionsMet(
            ...
            self.IsPartyMember,
            self.shared_memory_handler.get_agent_buffs(GLOBAL_CACHE.Player.GetAgentID()),
            self.shared_memory_handler.get_agent_buffs(vTarget)
        )
