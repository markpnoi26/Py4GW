...
from Sources.oazix.CustomBehaviors.primitives.helpers import blessing_helper_v2, custom_behavior_helpers
...
class TakeNearBlessingV2Utility(CustomSkillUtilityBase):
    def __init__(
            self,
            event_bus: EventBus,
        ...
        super().__init__(
            event_bus=event_bus,
            skill=CustomSkill("take_near_blessing_v2"),
    ...
    def _evaluate(self, current_state: BehaviorState, previously_attempted_skills: list[CustomSkill]) -> float | None:
        ...
        blessing_npc: tuple[blessing_helper_v2.BlessingNpcV2, int] | None = blessing_helper_v2.find_first_blessing_npc(Range.Earshot.value)
    ...
    def _execute(self, state: BehaviorState) -> Generator[Any, None, BehaviorResult]:
        ...
        lock_key = f"take_near_blessing_v2_{agent_id}"
    ...
    def run_dialog_sequence(self, agent_id: int) -> Generator[None, None, bool]:
        npc_dialog_visible = yield from blessing_helper_v2.wait_npc_dialog_visible(timeout_ms=3_500)
        ...
        result = yield from blessing_helper_v2.run_dialog_sequences(timeout_ms=3_500)
        if not result:
            if constants.DEBUG:
    ...
    def move_to_npc(self, agent_id: int) -> Generator[None, None, None]:
        ...
        if Utils.Distance(target_position, Player.GetXY()) > 150:
            ...
            yield from Routines.Yield.Movement.FollowPath(
                ...
                log=constants.DEBUG,
                timeout=10_000,
                progress_callback=lambda progress: print(f"FollowPath take_near_blessing_v2: progress: {progress}") if constants.DEBUG else None,
