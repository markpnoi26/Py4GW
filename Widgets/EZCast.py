...
def Draw():
    if not PyImGui.begin(MODULE_NAME, PyImGui.WindowFlags.NoFlag):
        return
    global cache
    #Styles for the Headers
    PyImGui.push_style_color(PyImGui.ImGuiCol.Header, (1, 0.1, 0.1, 0.5)) #ImGui::PushStyleColor(ImGuiCol_Header, sf::Color(0, 0, 0, 0));
    PyImGui.push_style_color(PyImGui.ImGuiCol.Border, (0.1, 0.1, 0.2, 1)) #ImGui::PushStyleColor(ImGuiCol_Border, sf::Color(255, 255, 255, 255));
    PyImGui.push_style_color(PyImGui.ImGuiCol.Text, (1, 1, 1, 1)) #ImGui::PushStyleColor(ImGuiCol_Text, sf::Color(255, 255, 255, 255));
...
def UseGenericSkills(energy, player_id, player_ag : GW.PyAgent.PyAgent, now):
    ...
    #TODO combat check isnt working
    combat = nearest_distance < square_dis
    for n in range(0,3):
        for i in range(1,9):
            if not cache.skill_array[n][i - 1]: continue
            if cache.combat_skill[i] and not combat: continue
