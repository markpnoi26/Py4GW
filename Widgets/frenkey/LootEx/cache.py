...
class Cached_Item:
    def __init__(self, item_id: int, slot: int = -1):
        ...
        self.rarity: Rarity = (
            Rarity(item.rarity.value) if item and item.rarity and item.rarity.value in Rarity else Rarity.White
        )
        self.profession: Profession = (
            Profession(item.profession) if item and item.profession in Profession else Profession._None
        )

        self.data: models.Item | None = (
            data.Items.get_item(self.item_type, self.model_id)
            if self.model_id > -1 and self.item_type in data.Items
            else None
        )
        ...
        self.color: DyeColor = utility.Util.get_color_from_info(self.dye_info) if self.dye_info else DyeColor.NoColor
        ...
        self.is_blacklisted: bool = (
            settings.profile.is_blacklisted(self.item_type, self.model_id)
            if settings.profile and self.model_id > -1
            else False
        )
        ...
        self.is_rare_weapon_to_keep: bool = (
            self.is_rare_weapon and settings.profile.rare_weapons.get(self.data.name, False)
            if self.data and settings.profile
            else False
        )

        mods_info = ItemModifiersInformation.GetModsFromModifiers(
            self.modifiers, self.item_type, self.model_id, self.is_inscribable
        )
        ...
        if settings.profile and settings.profile.skin_rules_by_model.get(self.item_type, {}).get(self.model_id, None):
            self.skin_rule = next(
                (
                    rule
                    for rule in settings.profile.skin_rules_by_model[self.item_type][self.model_id]
    ...
    def GetModsFromModifiers(self) -> tuple[list[RuneModInfo], list[WeaponModInfo]]:
        ...
        for identifier, arg1, arg2 in modifier_values:
            ...
            if identifier == ModifierIdentifier.TargetItemType.value:
                self.target_item_type = ItemType(arg1) if arg1 is not None else ItemType.Unknown
                self.is_rune = arg1 == 0 and arg2 == 0 and self.is_upgrade

            if identifier == ModifierIdentifier.Damage.value:
                self.damage = (arg2, arg1) if arg1 is not None and arg2 is not None else (0, 0)

            if identifier == ModifierIdentifier.Damage_NoReq.value:
                self.damage = (arg2, arg1) if arg1 is not None and arg2 is not None else (0, 0)

            if identifier == ModifierIdentifier.ShieldArmor.value:
                self.shield_armor = (arg1, arg2) if arg1 is not None and arg2 is not None else (0, 0)

            if identifier == ModifierIdentifier.Requirement.value:
                self.requirements = arg2 if arg2 is not None else 0
                self.attribute = Attribute(arg1) if arg1 is not None else Attribute.None_
        ...
        if self.is_weapon or (self.is_upgrade and not self.is_rune):
            ...
            self.weapon_mods_to_keep = [
                mod
                for mod in self.max_weapon_mods
                if settings.profile
                and settings.profile.weapon_mods.get(mod.WeaponMod.identifier, {}).get(self.item_type.name, False)
            ]
