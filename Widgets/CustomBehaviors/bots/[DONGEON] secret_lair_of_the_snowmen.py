...
class secret_lair_of_the_snowmen(BottingAbstract):
    ...
    def bot_routine(self, bot_instance: Botting):
        ...
        # critical failure is an un-recoverable state. it leads the bot to stop and no restart.
        def on_critical_failure() -> Generator[Any, Any, bool]:
            bot_instance.config.FSM.jump_to_state_by_name("[H]END_6")
            yield
            return False

        # failure is when a step is not behaving as expected. 
        # - If Map.IsExplorable : we will try trigger a resign - can leads to on_critical_failure if resign impossible.
        # - If Map.IsOutpost : we restart from beginning.
        def on_simple_failure() -> Generator[Any, Any, Any]:
            if GLOBAL_CACHE.Map.IsExplorable():
                yield from BottingHelpers.wrapper(action=BottingHelpers.wait_until_party_resign(timeout_ms = 50_000), on_failure=on_critical_failure)
            if GLOBAL_CACHE.Map.IsOutpost():
                bot_instance.config.FSM.jump_to_state_by_name("[H]MAIN_LOOP_1")
                yield

        def on_player_critical_stuck(event_message:EventMessage) -> Generator[Any, Any, Any]:
            if bot_instance.config.fsm_running:
                yield from on_simple_failure()

        # custom_behaviors event handling
        EVENT_BUS.subscribe(EventType.PLAYER_CRITICAL_STUCK, lambda event_message: on_player_critical_stuck(event_message))
        ...
        bot_instance.config.FSM.AddSelfManagedYieldStep("loot the dungeon lock key.", lambda: BottingHelpers.wrapper(action=BottingHelpers.wait_until_item_looted("Dungeon Key" ,timeout_ms = 15_000), on_failure=on_simple_failure))
        ...
        bot_instance.config.FSM.AddSelfManagedYieldStep("loot the boss key.", lambda: BottingHelpers.wrapper(action=BottingHelpers.wait_until_item_looted("Boss Key" ,timeout_ms = 15_000), on_failure=on_simple_failure))
        ...
        bot_instance.config.FSM.AddSelfManagedYieldStep( "wait for party resign.", lambda: BottingHelpers.wrapper(action=BottingHelpers.wait_until_party_resign(timeout_ms = 50_000), on_failure=on_critical_failure))
