...
class bogroot_growths(BottingAbstract):
    def __init__(self):
        super().__init__()
    ...
    def bot_routine(self, bot_instance: Botting):
        ...
        bot_instance.config.FSM.AddSelfManagedYieldStep("we are out of the outpost", lambda: BottingHelpers.wrapper(action=BottingHelpers.wait_until_on_map(558 , timeout_ms = 15_000), on_failure=self.botting_unrecoverable_issue))
    ...
    @override
    def name(self) -> str:
        return "[DONGEON] Bogroot Growths"

    @property
    @override
    def description(self) -> str:
        return "You cannot get past the first room unless everyone in the party has at least one of the two quests in their quest log. https://wiki.guildwars.com/wiki/Bogroot_Growths"