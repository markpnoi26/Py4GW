...
def draw_window():
    ...
    for cb in callback_list:
        cb_id, cb_name, cb_phase, cb_context, cb_priority, cb_order = cb
        phases.setdefault(cb_phase, []).append(cb)

    # order inside each phase
    for phase in phases:
        phases[phase].sort(key=lambda c: (c[4], c[5]))  # priority, order
    ...
    if PyImGui.begin("Callback Debug"):
        for phase, name in phase_names.items():
            ...
            if PyImGui.collapsing_header(f"{name} ({len(callbacks)})"):
                for cb_id, cb_name, cb_phase, cb_context, cb_priority, cb_order in callbacks:
                        PyImGui.text(
