from Py4GWCoreLib.enums import outpost_name_to_id, explorable_name_to_id

# 1) IDs
_4_yaksbend_to_icetoothcave_ids = {
    "outpost_id": outpost_name_to_id["Yaks Bend"],
}

# 2) Outpost exit path (inside Yak's Bend)
_4_yaksbend_to_icetoothcave_outpost_path = [
    (9295, 4145),
    (9280, 4050),
]

# 3) Segments - Reversed from the original _4_IceToothCave_To_YaksBend.py
_4_yaksbend_to_icetoothcave_segments = [
    {
        # "Traveler's Vale" explorable - Reversed from original
        "map_id": explorable_name_to_id["Traveler's Vale"],
        "path": [
            (8815.0, 2563.0),
            (8245.0, -696.0),
            (7710.0, -1050.0),
            (6752.0, -1858.0),
            (5425.0, -2008.0),
            (3109.0, -1709.0),
            (2631.0, -651.0),
            (2020.0, 2097.0),
            (2301, 4070),
            (2416, 5365),
            (323, 6484),
            (-1178, 8496),
            (-2256, 10265),
            (-3689, 11465),
            (-5982, 13184),
            (-6909, 13440),
            (-9661, 12612),
            (-10664, 13342),
            (-11000, 13909),
        ],
    },
    
    {
        # "Iron Horse Mine" explorable - Reversed from original
        "map_id": explorable_name_to_id["Iron Horse Mine"],
        "path": [
            (24176.0, -6004.0),
            (22756.0, -5700.0),
            (21327.0, -6185.0),
            (18670.0, -7501.0),
            (16780.0, -7984.0),
            (15656.0, -9822.0),
            (12910.0, -9237.0),
            (11356.0, -8246.0),
            (10607.0, -7184.0),
            (10483.0, -5693.0),
            (11861.0, -3197.0),
            (11562.0, -2281.0),
            (10422.0, -1769.0),
            (9368.0, -1843.0),
            (7797.0, -2406.0),
            (6317.0, -4240.0),
            (3044.0, -2868.0),
            (2207.0, -2415.0),
            (35.0, -3038.0),
            (-3211.0, -406.0),
            (-6424.0, 2034.0),
            (-7758.0, 2093.0),
            (-10259.0, 1925.0),
            (-12206.0, 3366.0),
            (-14121.0, 4485.0),
            (-14828.0, 6901.0),
            (-15400.0, 7600.0),
            (-16146.0, 7772.0),
            (-16884.0, 7570.0),
            (-18837.0, 6882.0),
            (-23489.0, 5823.0),
            (-24189.0, 4854.0),
            (-23984.0, 3299.0),
            (-24537.0, 2634.0),
            (-25225, 2312),
            (-25700, 1950),
        ],
    },
    
    {
        # "Anvil Rock" explorable - Reversed from original
        "map_id": explorable_name_to_id["Anvil Rock"],
        "path": [
            (19364.0, 19008.0),
            (17995.0, 17903.0),
            (15838.0, 19786.0),
            (13942.0, 19818.0),
            (13253.0, 19607.0),
            (12475.0, 19055.0),
            (10298.0, 17573.0),
            (9134.0, 16979.0),
            (8042.0, 16927.0),
            (4887.0, 16724.0),
            (1931.0, 16377.0),
            (-492.0, 16438.0),
            (-3509.0, 16144.0),
            (-7024.0, 16058.0),
            (-8753.0, 15967.0),
            (-10114.0, 15485.0),
            (-12797.0, 13737.0),
            (-13344.0, 13028.0),
            (-13106.0, 12483.0),
            (-12842, 12028),
            (-12350,11667),
        ],
    },
]
