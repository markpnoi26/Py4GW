from Py4GWCoreLib.enums import outpost_name_to_id, explorable_name_to_id

# 1) IDs
_6_beaconsperch_to_gatesofkryta_ids = {
    "outpost_id": outpost_name_to_id["Beacons Perch"],
}

# 2) Outpost exit path
_6_beaconsperch_to_gatesofkryta_outpost_path = [
    (-10472, 35025),  
    (-10800, 35300), 
]

# 3) Segments - Reversed from the original _2_GatesOfKryta_To_BeaconsPerch.py
_6_beaconsperch_to_gatesofkryta_segments = [
    {
        # "Deldrimor Bowl" explorable (reversed)
        "map_id": explorable_name_to_id["Deldrimor Bowl"],
        "path": [
            (12956, -21183),
            (13489.0, -20385.0),
            (14704.0, -18406.0),
            (15423.0, -16147.0),
            (15451.0, -14152.0),
            (14207.0, -13664.0),
            (11838.0, -13890.0),
            (10729.0, -14183.0),
            (9273.0, -15006.0),
            (6573.0, -16422.0),
            (5214.0, -18898.0),
            (3842.0, -19936.0),
            (2164.0, -20283.0),
            (488.0, -19996.0),
            (-1004.0, -19201.0),
            (-3336.0, -17886.0),
            (-5124.0, -17194.0),
            (-6667.0, -17995.0),
            (-7072.0, -20626.0),
            (-7426.0, -21513.0),
            (-8296.0, -22449.0),
            (-9584.0, -22665.0),
            (-10602.0, -22156.0),
            (-11603.0, -22051.0),
            (-12738.0, -22106.0),
            (-13602, -22185),
            (-13900, -23100)
        ],
    },
    {
        # "Griffon's Mouth" explorable (reversed)
        "map_id": explorable_name_to_id["Griffon's Mouth"],
        "path": [
            (7492, 7030),
            (6866.0, 6484.0),
            (6402.0, 6454.0),
            (5388.0, 6979.0),
            (4338.0, 7536.0),
            (3382.0, 8245.0),
            (2418.0, 7916.0),
            (981.0, 6553.0),
            (-247.0, 6249.0),
            (-652.0, 5395.0),
            (-1256.0, 4209.0),
            (-2074.0, 4142.0),
            (-3718.0, 1107.0),
            (-3641.0, 184.0),
            (-2885.0, -1328.0),
            (-1570.0, -3222.0),
            (-1789.0, -4126.0),
            (-1805.0, -4921.0),
            (-1440.0, -5698.0),
            (-1521.0, -6687.0),
            (-2531.0, -7179.0),
            (-3939.0, -7119.0),
            (-5804.0, -6097.0),
            (-7062, -6445),
            (-7610, -7500)           
        ],
    },
    {
        # "Scoundrel's Rise" explorable (reversed)
        "map_id": explorable_name_to_id["Scoundrel's Rise"],
        "path": [
            (7541.0, 5537.0),
            (6468.0, 3577.0),
            (5915.0, 2336.0),
            (5176.0, 773.0),
            (3907.0, -1574.0),
            (2635.0, -2237.0),
            (116.0, -2720.0),
            (-1047.0, -3593.0),
            (-1876.0, -4738.0),
            (-1970.0, -5314.0),
            (-1418.0, -6793.0),
            (-1280.0, -7015.0)
        ],
    },
    
]
