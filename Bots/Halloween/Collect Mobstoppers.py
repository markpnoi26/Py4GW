...
def create_bot_routine(bot: Botting) -> None:
    ...
    for _ in range(iterations):
        ...
        TakeQuest(bot)
        bot.Quests.RequestInfo(QUEST_ID, False)
        bot.Wait.ForTime(1)
        bot.Quests.WaitActive(QUEST_ID, timeout_ms=1, step_ms=1)
        AbandonQuest(bot)
...
def AbandonQuest(bot: Botting):
    bot.States.AddHeader("Abandon Quest")

    def _abandon_gen():
        bot.Quests.SetActive(QUEST_ID)
        ...
        if bot.Quests.GetActive() == QUEST_ID:
            bot.Quests.Abandon(QUEST_ID)
            yield from Routines.Yield.wait(1)
            bot.Quests.RequestInfo(QUEST_ID, False)
            yield from Routines.Yield.wait(1)
