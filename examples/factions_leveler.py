...
class FSM_Config:
    ...
    def initialize(self):
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Shing Jea Monastery")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Linnok Courtyard")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Sunqua Vale")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Kinya Province")),
        )  # 236
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Tsumei Village")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Panjiang Peninsula")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            name="Wait for Seitung Harbor Map Load",
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Seitung Harbor")),
        )
        ...
        self.FSM.AddYieldRoutineStep(
            ...
            coroutine_fn=lambda: self.WaitforMapLoad(GLOBAL_CACHE.Map.GetMapIDByName("Haiju Lagoon")),
        )
    ...
    def follow_path(self, path: List[Tuple[float, float]], pause_on_danger: bool = False) -> Generator[Any, Any, bool]:
        success_movement = yield from Routines.Yield.Movement.FollowPath(
            ...
            log=False,
            custom_pause_fn=(
                (lambda: Routines.Checks.Agents.InDanger(aggro_area=Range.Earshot)) if pause_on_danger else None
            ),
        )
    ...
    def CraftWeapons(self):
        path_to_crafter: List[Tuple[float, float]] = [
            (-3869, 10346),
            (-3943, 11015),
            (-4062, 11863),
            (-4722, 12376),
        ...
        if profession in MELEE_CLASSES:
            ...
            if iron_in_storage < 5:
                Py4GW.Console.Log(
                    MODULE_NAME, "Not enough Iron Ingots to craft weapons.", Py4GW.Console.MessageType.Error
                )
                yield from self._stop_execution()
                return
            ...
            if not items_witdrawn:
                Py4GW.Console.Log(
                    MODULE_NAME, "Failed to withdraw Iron Ingots from storage.", Py4GW.Console.MessageType.Error
                )
                yield from self._stop_execution()
                return
    ...
    def MinisterChoMission(self):
        ...
        try:
            ...
            path_to_mission: List[Tuple[float, float]] = [
                (6358, -7348),
                (5444, -7748),
                (5254, -7879),
                (4170, -8836),
    ...
    def TheThreatGrows(self):
        ...
        try:
            ...
            while True:
                ...
                if (not Routines.Checks.Agents.InDanger(aggro_area=Range.Spellcast)) and GLOBAL_CACHE.Agent.HasQuest(
                    sister_tai_agent_id
                ):
                    break
                yield from Routines.Yield.wait(1000)
