...
class UIMessage(IntEnum):
    ...
    # NOTE: Python had these shifted — corrected to match C++
    kQuestAdded                  = 0x1000014C
    kQuestDetailsChanged         = 0x1000014D  # wparam = uint32_t quest_id
    kQuestRemoved                = 0x1000014E  # wparam = uint32_t quest_id
    kClientActiveQuestChanged    = 0x1000014F  # wparam = uint32_t quest_id
    kServerActiveQuestChanged    = 0x10000151
    kUnknownQuestRelated         = 0x10000152  # wparam = uint32_t quest_id

    kDungeonCompleted            = 0x10000154
    kMissionCompleted            = 0x10000155
    kVanquishComplete            = 0x10000157

    kObjectiveAdd                = 0x10000158  # wparam = uint32_t objective_id
    kObjectiveComplete           = 0x10000159  # wparam = uint32_t objective_id
    kObjectiveUpdated            = 0x1000015A  # wparam = uint32_t objective_id

    kTradeSessionStart           = 0x10000163
    kTradeSessionUpdated         = 0x10000169
    kTriggerLogoutPrompt         = 0x1000016F
    kToggleOptionsWindow         = 0x10000170
    kCheckUIState                = 0x10000173
    kRedrawItem                  = 0x10000175  # wparam = uint32_t item_id
    kCloseSettings               = 0x10000177
    kChangeSettingsTab           = 0x10000178  # wparam = uint32_t tab_id
    kGuildHall                   = 0x1000017A
    kLeaveGuildHall              = 0x1000017C
    kTravel                      = 0x1000017D
    kOpenWikiUrl                 = 0x1000017E  # wparam = wchar_t* url

    kAppendMessageToChat         = 0x1000018C  # wparam = wchar_t* message
    kHideHeroPanel               = 0x1000019A
    kShowHeroPanel               = 0x1000019B
    kGetInventoryAgentId         = 0x100001A0
    kEquipItem                   = 0x100001A1  # wparam = uint32_t item_id
    kMoveItem                    = 0x100001A2  # wparam = UIPacket::kMoveItem*
    kInitiateTrade               = 0x100001A4
